...
async function loadSpaceUIConfigs() {
  try {
    ...
    document.getElementById('admin-space-admin-password').value = data.adminPasswordPlain || "";
...
async function loadGameConfigs() {
  try {
    ...
    if (games && games.length > 0) {
      ...
      if (viewsEl) viewsEl.textContent = game.views || 0;

      const completionsEl = document.getElementById('stats-completions');
      if (completionsEl) completionsEl.textContent = game.completions || 0;

      const startDateEl = document.getElementById('admin-start-date');
      if (startDateEl && game.startDate) {
        startDateEl.value = new Date(game.startDate).toISOString().slice(0, 16);
      }

      const expiryDateEl = document.getElementById('admin-expiry-date');
      if (expiryDateEl && game.expiryDate) {
        expiryDateEl.value = new Date(game.expiryDate).toISOString().slice(0, 16);
      }
...
function setupActionListeners() {
  ...
  document.getElementById('admin-btn-save').onclick = async () => {
    ...
    try {
      ...
      const startDateVal = document.getElementById('admin-start-date')?.value || null;
      const expiryDateVal = document.getElementById('admin-expiry-date')?.value || null;
  ...
  document.getElementById('form-update-passwords').onsubmit = async (e) => {
    ...
    const adminPassword = document.getElementById('admin-space-admin-password').value.trim();
...
function updateShareLinkUI() {
  ...
  const gameLink = slug ? `${window.location.origin}/${slug}` : `${window.location.origin}/${currentGameId}`;
...
function initFlatpickr() {
  if (typeof window.flatpickr !== 'undefined') {
    ...
    if (document.getElementById('admin-start-date') || document.getElementById('admin-expiry-date')) {
